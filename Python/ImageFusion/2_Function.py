...
root = Tk()
root.title("JIHUN IMAGE FUSION")

# Add/Del
def add_file():
    files = filedialog.askopenfilenames(title="Select Image",
                                         filetypes=(("JPG File", "*.jpg"),("All Files", "*.*")),
                                         initialdir="C:/Users/visio/OneDrive/Desktop/Python/ImageFusion/image")
    for file in files:
...
def browse_dest_path():
    folder_selected = filedialog.askdirectory(initialdir="C:/Users/visio/OneDrive/Desktop/Python/ImageFusion/image")
    if folder_selected == "":
        return
...
def fusion_images():
    ...
    for idx, img in enumerate(images) :
        if img_width > -1:
            img = img.rexize(image_sizes[idx])
...
btn_add_file = Button(file_frame,text="Delete", command=del_file)
...
btn_close = Button(frame_run, padx=5, pady=5, text="Start", width=12, command=root.quit)
